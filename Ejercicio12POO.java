...
public class Ejercicio12POO {
    ...
    public static void main(String[] args) {
        ...
        ps.crearPersona(p);
        System.out.println("La edad que tiene hoy el usuario "+p.getNombre()+" es de "+ps.calcularEdad(p));
        
        
        System.out.println("Ingrese edad a comparar");
        int edad= leer.nextInt();
        boolean menor = ps.menorQue(ps.calcularEdad(p), edad);
        if (menor==true){
            System.out.println("La edad ingresada nueva es menor que la Edad de la persona "+p.getNombre());
        } else {
            System.out.println("La edad ingresada nueva es mayor que la edad de la persona "+p.getNombre());
        }
        ps.mostrarPersona(p);
    }
